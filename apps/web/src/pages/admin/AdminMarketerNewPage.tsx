...
export function AdminMarketerNewPage() {
  ...
  const create = useMutation({
    mutationFn: () => api.post<{ marketer: { id: string } }>('/admin/marketers', {
      ...form,
      code: form.code.toUpperCase(),
      commissionRate: parseFloat(form.commissionRate) / 100,
    }),
    onSuccess: (data: { marketer?: { id: string }; id?: string }) => {
      qc.invalidateQueries({ queryKey: ['admin-marketers'] });
      const mid = data?.marketer?.id ?? data?.id;
      if (mid) navigate({ to: '/admin/marketers/$id', params: { id: mid } });
      else navigate({ to: '/admin/marketers' });
    },
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <h1 className="text-2xl font-semibold text-slate-900">Yeni Pazarlamacı</h1>
      <Card>
        <form onSubmit={(e) => { e.preventDefault(); create.mutate(); }} className="grid gap-4 sm:grid-cols-2">
          <Field label="Firma adı"><Input placeholder="Firma adı" value={form.companyName} onChange={(e) => setForm({ ...form, companyName: e.target.value })} required /></Field>
          <Field label="Kod"><Input placeholder="Kod" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} required /></Field>
          ...
          <Field label="Komisyon %"><Input placeholder="20" type="number" value={form.commissionRate} onChange={(e) => setForm({ ...form, commissionRate: e.target.value })} /></Field>
          ...
          <Button type="submit" className="sm:col-span-2" disabled={create.isPending}>Oluştur</Button>
        </form>
      </Card>
    </div>
  );
