...
export function JobStatusPage() {
  ...
  const [data, setData] = useState<{
    ...
    position: string;
    createdAt: string;
    events: Array<{ type: string; note: string | null; createdAt: string }>;
  } | null>(null);
  ...
  const check = async () => {
    ...
    try {
      const result = await api.get<typeof data>(
        `/recruitment/status/${encodeURIComponent(trackingCode)}?phone=${encodeURIComponent(phone)}`,
      );
      setData(result);
    } catch (err) {
      ...
      <div className="mx-auto w-full max-w-lg flex-1 px-4 py-10">
        ...
        <Card className="mt-6 shadow-elevated">
          <div className="space-y-4">
            <Input placeholder="Takip Kodu" value={trackingCode} onChange={(e) => setTrackingCode(e.target.value)} />
          ...
          {data && (
            <div className="mt-6 space-y-4 border-t border-slate-100 pt-6">
              <div className="flex items-center justify-between">
                ...
                <Badge variant={data.status === 'HIRED' ? 'success' : data.status === 'REJECTED' ? 'error' : 'warning'}>
                ...
                <ol className="mt-2 space-y-2">
                  {data.events.map((e, i) => (
                    <li key={i} className="text-xs text-slate-600">
                      <span className="font-medium">{STATUS_LABELS[e.type] || e.type}</span> — {e.note || '-'}
                      <br />
